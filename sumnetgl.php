...
$q3="SELECT `qni`,`qtr`,`qtc`,`manhours`,`period`,`sessions` " .
  "FROM `netreport` WHERE `period` BETWEEN " . $p1 . " AND " . $p9 .
  " ORDER BY `period`";
$r3=getResult($q3,$db);
...
while ( $row3 = getRow($r3,$db) )
  {
	...
	$hours = $row3[3];
	if ( $hours == 0 )
	  $hours = $row3[0]*$row3[1]/$row3[5];
	$y1[$i] = $y1[$i]+$hours; // man hours
	$y2[$i] = $y2[$i]+$row3[0]; // qni
	$y3[$i] = $y3[$i]+$row3[1]; // qtr
	$y4[$i] = $y4[$i]+$row3[2]; // qtc
      }
  }
...
for ( $i=1; $i<$n; $i++ )
{
    ...
    ImageLine( $image, $xp[$i], $yp4[$i]-1, $xp[$i+1], $yp4[$i+1]-1, $line4 );
    ImageLine( $image, $xp[$i], $yp4[$i]+1, $xp[$i+1], $yp4[$i+1]+1, $line4 );
    ImageLine( $image, $xp[$i], $yp4[$i], $xp[$i+1], $yp4[$i+1], $line4 );
    ImageLine( $image, $xp[$i]-1, $yp4[$i], $xp[$i+1]-1, $yp4[$i+1], $line4 );
    ImageLine( $image, $xp[$i]+1, $yp4[$i], $xp[$i+1]+1, $yp4[$i+1], $line4 );
...
ImageString($image, 3, $gr+5, 60,  'Manhours', $line1);
ImageString($image, 3, $gr+5, 80,  'Checkins',     $line2);
ImageString($image, 3, $gr+5, 100, 'Time',  $line3);
ImageString($image, 3, $gr+5, 120, 'Messages',  $line4);
ImageString($image, 1, 290, 210, 'sumnetgl.php $Revision: 1.0 $', $ltgray );
ImageString($image, 3, $gr/2-20, $gb+20, 'Period', $black );
ImageString($image, 3, $gr/2-60, $gt-20, 'Affiliated Net Results', $black );
