...
$q3="SELECT `cat4`,`cat5`,`total`,`period` " .
  "FROM `pshr` WHERE `period` BETWEEN " . $p1 . " AND " . $p9 .
  " ORDER BY `period`";
$r3=getResult($q3,$db);
...
while ( $row3 = getRow($r3,$db) )
  {
	...
	$i = $row3[3]-$p1;
	$x[$i] = $row3[3];  // Period
	$y1[$i] = $y1[$i]+$row3[2]; // Total hours
	$y2[$i] = $y2[$i]+$row3[0]; // Public Service
	$y3[$i] = $y3[$i]+$row3[1]; // Emergency
      }
  }
...
ImageString($image, 3, $gr+5, 60,  'Score', $line1);
ImageString($image, 3, $gr+5, 90,  'Pub.Svc.',     $line2);
ImageString($image, 2, $gr+5, 105, '   (x5)',     $line2);
ImageString($image, 3, $gr+5, 120, 'Emerg',  $line3);
ImageString($image, 2, $gr+5, 135, '   (x5)',  $line3);
ImageString($image, 3, $gr/2-20, $gb+20, 'Period', $black );
ImageString($image, 3, $gr/2-80, $gt-20, 'Public Service Honor Roll', $black );
ImageString($image, 1, 290, 210, 'sumpshrgl.php $Revision: 1.0 $', $ltgray );
